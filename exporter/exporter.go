...
func (cfg *Config) Start(listenAddress, version string) {
	...
	router.GET("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		tmpl.Execute(w, &indexVariables{
			Instance: cfg.instance.String(),
			Version:  version,
		})
	})

	router.GET("/apgroups", cfg.listAPGroups)
	router.GET("/apgroups/:ap_group/debug", cfg.debugHandler)
...
func (cfg *Config) listAPGroups(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	r.Body.Close()
	result, err := cfg.fetchAPGroups(r.Context())
	if err != nil {
...
func (cfg *Config) debugHandler(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	apg := params.ByName("ap_group")

	devices, err := cfg.fetchDevices(r.Context(), apg)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadGateway)
	...
	basic, err := cfg.fetchAPGroupData(r.Context(), apg)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadGateway)
